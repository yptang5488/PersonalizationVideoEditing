...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        if temp_embed:
            print(f'[VisionTransformer temp_embed] num_frames, width = {num_frames}, {width}')
            self.temporal_positional_embedding = nn.Parameter(torch.zeros(1, num_frames, width))
        # print(f'[INFO : VisionTransformer] self.temporal_positional_embedding.shape = {self.temporal_positional_embedding.shape}')
    ...
    def forward(self, x, masking_prob=0.0):
        ...
        print(f'[INFO] self.temporal_positional_embedding.shape = {self.temporal_positional_embedding.shape}')
        print(f'[INFO] self.temporal_positional_embedding.mean(1).shape = {self.temporal_positional_embedding.mean(1).shape}')
